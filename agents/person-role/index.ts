...
const PORT = Number(process.env["CONNECTOR_PORT"] ?? 8795);
const GDELT_ENABLED = (process.env["SIGNALS_GDELT"] ?? "on").toLowerCase() !== "off";

const NAME = "person-role — Person and role intelligence —";
const SPECIALTY =
  "Person and role intelligence — decision makers and professional relationships";
 ...
 *
 * person-role: Who decides. Leadership moves and hiring signal strategy shifts earlier than product news. EDGAR on for 8-K Item 5.02 officer changes.
 */
const BEAT = {
  googleNews: true,
  gdelt: false,
  ...
  angles: [
    "appointed chief executive hire",
    "executive departure resigns",
    "board appointment",
  ] as string[],
};

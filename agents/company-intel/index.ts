...
const PORT = Number(process.env["CONNECTOR_PORT"] ?? 8794);
const GDELT_ENABLED = (process.env["SIGNALS_GDELT"] ?? "on").toLowerCase() !== "off";

const NAME = "company-intel — Company intelligence — organiz";
const SPECIALTY =
  "Company intelligence — organizations, capacity and expansion signals";
 ...
 *
 * company-intel: Company fundamentals from wire services and primary filings. EDGAR on, because a registrant's own 8-K outranks any story about it.
 */
const BEAT = {
  googleNews: true,
  gdelt: false,

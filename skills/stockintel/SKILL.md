...
Then call `stockintel_investigate` first for a live thesis: the full ten-specialist grid investigates and the orchestrator connects event to exposure to verdict. A full run takes about 7 minutes. Poll `stockintel_inquiry` every 30 seconds until status is complete, and never substitute a stored read for the live run while it is still open. Falling back to `stockintel_assess` mid-run reports a different run's answer as this run's. For previous thinking, `stockintel_assess` serves the latest thesis, `stockintel_clusters` the grouped evidence, `stockintel_thesis_changes` what moved, `stockintel_conflicting` the counter-case, `stockintel_evidence` one thread drilled down. Stored reads admit their age past a day. `stockintel_read` covers market context, `stockintel_status` the deployment state.
